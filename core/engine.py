...
class CognitiveEngine:
    ...
    def __init__(self, db_path: str | Path | None = None) -> None:
        ...
        # first child -> possible existing parent compositions
    ...
    def _record_composition(self, parent_id: str, child_ids: list[str]) -> None:
        if self._composition_exists(parent_id, child_ids):
            return
        ...
        # Parent composition lookup changed. Refresh lazily.
    ...
    def _composition_candidates(self, first_child_id: str) -> list[CompositionCandidate]:
        ...
        # If multiple known structures fit the same place, the structurally
        # higher / wider existing Unit is the most complete projection.
    ...
    def activate_sentence(self, sentence_id: str) -> dict[str, Any]:
        ...
        Newly learned Units elsewhere may appear in this active view, but the
        historical sentence rows themselves are never modified.
        """
            ...
            layers.append(
                {
                    "level": 0,
                    "unit_ids": current,
                    "contents": [self.get_unit_content(uid) for uid in current],
                    "depths": [self.get_unit_depth(uid) for uid in current],
                }
            )
    ...
    def _discover_from_density(
        ...
        For every adjacent pair, density is simply the number of historical
        CellElements containing that exact address transition. A positive count
        means the current observation is laying another sheet on an already
        observed connection. Consecutive visible connections form a new block.
        """
        if len(unit_ids) <= 1:
            return list(unit_ids), []
    ...
    def process_sentence(self, raw_text: str) -> dict[str, Any]:
        ...
        while pass_id < 100:
            # Existing higher Units are projected only on this active sequence.
            projected_ids = self._project_existing_units(current_unit_ids)
            # New structure is learned from literal cell overlap counts.
            segmented_ids, densities = self._discover_from_density(
                projected_ids, sentence_id
            )

            # Persist only the layer that was actually observed at this pass.
