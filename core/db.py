...
CREATE INDEX IF NOT EXISTS idx_units_depth_content
ON units (depth, content);
...
CREATE INDEX IF NOT EXISTS idx_compositions_parent
ON compositions (parent_unit_id);

CREATE INDEX IF NOT EXISTS idx_compositions_child
ON compositions (child_unit_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_cell_elements_observation
ON cell_elements (
    source_unit_id,
    next_unit_id,
    sentence_id,
...
CREATE INDEX IF NOT EXISTS idx_cell_elements_sentence_position
ON cell_elements (sentence_id, position);

CREATE INDEX IF NOT EXISTS idx_cell_elements_depth
ON cell_elements (cell_depth);
"""
...
class Database:
    ...
    def _migrate_legacy_unit_links(self) -> None:
        """
        Copy old unit_links rows into cell_elements once.

        The old table is intentionally left untouched so an existing development DB
        can be opened safely. INSERT OR IGNORE plus the unique observation index makes
